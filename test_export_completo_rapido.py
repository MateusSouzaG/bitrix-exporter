"""Teste completo de exportação com amostra limitada."""
...
def test_export_completo():
    """Testa o fluxo completo de exportação."""
    print("=" * 60)
    print("TESTE COMPLETO DE EXPORTACAO (AMOSTRA)")
    print("=" * 60)
    ...
    # Limitar a 20 para teste rápido
    task_ids = set(list(all_task_ids)[:20])
    print(f"Processando amostra de {len(task_ids)} tarefas...")
    
    # Enriquecer tarefas
    print(f"\nEnriquecendo tarefas...")
    ...
    if len(excel_rows) == 0:
        print("\n[PROBLEMA] Nenhuma linha foi gerada!")
        return
    
    # Gerar Excel
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_path = f"teste_export_{timestamp}.xlsx"
    print(f"\nGerando Excel: {output_path}")
    write_tasks_excel(excel_rows, output_path)
    
    print(f"\n[OK] Exportação concluída!")
    print(f"   Arquivo: {output_path}")
    print(f"   Linhas: {len(excel_rows)}")
    print(f"\n[INFO] Total de tarefas disponíveis: {len(all_task_ids)}")
    print(f"       Para exportar todas, use a interface web.")

if __name__ == "__main__":
    test_export_completo()
